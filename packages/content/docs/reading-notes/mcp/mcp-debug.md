...
对于 remote 类型的 MCP server，我们还可以使用抓包工具来调试，比如 Wireshark、Proxyman、Fiddler 等等。对于 HTTP 流量，我们直接 Wireshark 就可以，能够清晰的看到请求和响应的内容。对于已经上线的 HTTPS 服务，我们可以使用 Proxyman 这类支持 HTTPS 解密的抓包工具来进行调试，但还是有一些需要注意的点

之前在调试 streamable HTTP 的 MCP server 时，发现 Proxyman 抓不到请求。排查后确认原因是：Node 内置的 fetch 默认不会走系统代理，SDK 又是基于 fetch 实现的，所以需要显式注入带代理能力的 fetch。

我的实践结论是：修改 MCP client 的源码。我在 Streamable HTTP 连接创建时注入 undici 的 ProxyAgent，并支持从 mcp.json 里配置代理地址。

1. 使用 undici 代理 fetch（伪代码）

```ts
import { fetch as undiciFetch, ProxyAgent } from "undici";

const proxyAgent = new ProxyAgent("http://127.0.0.1:9090");
const proxyFetch = (input, init) =>
  undiciFetch(input, { ...init, dispatcher: proxyAgent });
```

![image.png](https://2f0f3db.webp.li/2025/12/20251224211227522.png)
...
- system proxy 对 Node fetch 无效，必须显式注入代理能力
- mcp.json 增加 proxy 配置是最直观的方式，但这需要 MCP 客户端的配合
